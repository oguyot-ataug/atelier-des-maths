...
/* ======================= Lien d'invitation (choix du mot de passe) =======================
   Signalé : "on pourrait avoir un lien personnalisé ?" -- au lieu de communiquer un mot de
   passe en clair, l'élève clique sur un lien qui lui est propre et choisit lui-même son mot
   de passe. Les 2 actions serveur utilisées (check-invitation, consume-invitation) sont
   volontairement PUBLIQUES (pas besoin d'être connecté, l'élève ne l'est justement pas
   encore) -- mais la passerelle Supabase exige tout de même un JWT valide en en-tête, d'où
   l'utilisation de la clé publique anonyme (déjà exposée partout ailleurs sur ce site). */
let invitationToken = null;
async function checkForInvitationLink(){
  ...
  try{
    const res = await fetch(SUPABASE_URL+'/functions/v1/admin-create-user', {
      ...
      body: JSON.stringify({ action:'check-invitation', token }),
    });
    const data = await res.json();
    if(data.error){
      titleEl.textContent = 'Lien invalide';
      subtitleEl.textContent = data.error;
      return true;
    }
    titleEl.textContent = data.nom ? `Bienvenue, ${data.nom} !` : 'Bienvenue !';
    subtitleEl.textContent = 'Choisissez votre mot de passe pour activer votre compte.';
    document.getElementById('invitationFormZone').style.display = 'block';
  }catch(err){
    titleEl.textContent = 'Erreur';
    subtitleEl.textContent = 'Impossible de vérifier ce lien pour le moment (problème réseau ?).';
  }
  return true;
}
async function submitInvitation(){
  const pass1 = document.getElementById('invitationPassword').value;
  const pass2 = document.getElementById('invitationPasswordConfirm').value;
  const status = document.getElementById('invitationStatus');
  if(pass1.length<6){ status.textContent = 'Mot de passe trop court (6 caractères minimum).'; return; }
  if(pass1!==pass2){ status.textContent = 'Les deux mots de passe ne correspondent pas.'; return; }
  ...
  try{
    const res = await fetch(SUPABASE_URL+'/functions/v1/admin-create-user', {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'Authorization': 'Bearer '+SUPABASE_ANON_KEY },
      body: JSON.stringify({ action:'consume-invitation', token: invitationToken, newPassword: pass1 }),
    });
    const data = await res.json();
    if(data.error){ status.textContent = "Erreur : "+data.error; return; }
    status.textContent = 'Compte activé, connexion en cours…';
    const { error } = await sb.auth.signInWithPassword({ email: data.email, password: pass1 });
    if(error){ status.textContent = "Mot de passe enregistré, mais la connexion automatique a échoué : "+error.message+" Essayez de vous connecter manuellement."; return; }
    // Retire le paramètre ?invite= de l'URL et retourne à l'accueil, désormais connecté.
    history.replaceState(null, '', location.pathname);
    showView('view-home');
  }catch(err){ status.textContent = 'Erreur réseau, réessayez.'; }
}
