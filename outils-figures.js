...
function renderLengthCode(p1,p2,group,color){
  ...
  for(let i=0;i<group;i++){
    ...
    html+=`<line x1="${cx-tx*3}" y1="${cy-ty*3}" x2="${cx+tx*3}" y2="${cy+ty*3}" stroke="${color}" stroke-width="0.7"/>`;
  }
  return html;
}
function renderAngleCode(vertex,p1,p2,group,color){
  ...
  let html = `<polyline points="${points}" fill="none" stroke="${color}" stroke-width="0.8"/>`;
  ...
  const angSpacing = 0.17, offsetStart = -((group-1)*angSpacing)/2;
  for(let i=0;i<group;i++){
    const a = mid+offsetStart+i*angSpacing;
    const cx=vertex.x+r*Math.cos(a), cy=vertex.y+r*Math.sin(a);
    const nx=Math.cos(a), ny=Math.sin(a);
    html+=`<line x1="${(cx-nx*2.5).toFixed(1)}" y1="${(cy-ny*2.5).toFixed(1)}" x2="${(cx+nx*2.5).toFixed(1)}" y2="${(cy+ny*2.5).toFixed(1)}" stroke="${color}" stroke-width="0.7"/>`;
  }
  return html;
}
function renderRightAngleCode(vertex,p1,p2){
  ...
  return `<polyline points="${c1.x},${c1.y} ${c3.x},${c3.y} ${c2.x},${c2.y}" fill="none" stroke="#E35D3A" stroke-width="0.8"/>`;
}
...
function shapeStrokeAttrs(s, defaultColor){
  ...
  const sw = s.strokeWidth==='epais' ? 1.2 : 0.7;
...
function renderFigureSvg(){
  ...
  figState.shapes.forEach(s=>{
    if(s.type==='segment'){
      ...
      if(s.lengthLabel){
        ...
        html+=`<text x="${mx}" y="${my-9}" font-family="JetBrains Mono" font-size="9" fill="#5C5A78" text-anchor="middle">${s.lengthLabel}</text>`;
      }
      ...
      const ux=dx/len, uy=dy/len, nx=-uy, ny=ux, headLen=5.5, headW=2.5;
      ...
      if(s.radiusLabel) html+=`<text x="${s.p1.x+(refPoint.x-s.p1.x)/2}" y="${s.p1.y+(refPoint.y-s.p1.y)/2-8}" font-family="JetBrains Mono" font-size="9" fill="#5C5A78" text-anchor="middle">${s.radiusLabel}</text>`;
      if(s.codeGroup){
      ...
      html+=`<text x="${lx}" y="${ly}" font-family="JetBrains Mono" font-size="9" fill="#E35D3A" text-anchor="middle">${angleDegrees(s.vertex,s.p1,s.p2)}°</text>`;
      ...
      html += `<text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" font-family="JetBrains Mono" font-size="9" fill="${s.strokeColor||'#8A2F52'}" text-anchor="middle" transform="rotate(${textAngle.toFixed(1)} ${lx.toFixed(1)} ${ly.toFixed(1)})">${cm} cm</text>`;
    } else if(s.type==='mesure-distance-ligne'){
  ...
  figState.points.forEach(p=>{
    ...
    if(linked.length===0){
      html+=`<line x1="${p.x-2.5}" y1="${p.y-2.5}" x2="${p.x+2.5}" y2="${p.y+2.5}" stroke="${c}" stroke-width="0.9"/>`;
      html+=`<line x1="${p.x-2.5}" y1="${p.y+2.5}" x2="${p.x+2.5}" y2="${p.y-2.5}" stroke="${c}" stroke-width="0.9"/>`;
